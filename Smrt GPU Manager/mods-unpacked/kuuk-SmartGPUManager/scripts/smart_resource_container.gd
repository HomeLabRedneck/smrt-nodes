...
func _ready() -> void:
    super()
    data_changed = true
    connection_out_set.connect(func(): update_connections())
...
func tick() -> void:
    # Fallback: sync transfer from outputs if update_connections() didn't fire
    var _transfer: Array = transfer if "transfer" in self else []
    if _transfer.is_empty() and not outputs.is_empty():
        update_connections()
        _transfer = transfer if "transfer" in self else []
...
func _update_wdata(target_inputs: Array, wdata: Dictionary):
